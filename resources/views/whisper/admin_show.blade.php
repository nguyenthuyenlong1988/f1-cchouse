...
@section('page_title', 'Tâm sự #' . $post->id)
  ...
  <div class="panel panel-primary">
    <div class="panel-heading">
      <div class="btn-group" role="group">
        <a class="btn btn-primary" href="{{ route('admin::@dmin-zone.whisper.index') }}"><span class="glyphicon glyphicon-menu-left"></span></a>
        <a class="btn btn-primary" href="{{ route('admin::@dmin-zone.whisper.edit', $post->id) }}">Chỉnh sửa</a>
      </div>
      <div class="btn-group">
        <a class="btn btn-primary" href="{{ route('admin::@dmin-zone.whisper.create') }}">Tâm sự mới</a>
        ...
        <ul class="dropdown-menu">
          <li>
            <a href="javascript:void(0)" onclick="_func.cfmOnDeletePost('{{ $post->id }}', '{{ $post->post_title }}', '{{ csrf_token() }}')" style="color:#c9302c">
              <span class="glyphicon glyphicon-trash" aria-hidden="true"></span> Xóa tâm sự
            </a>
          </li>
        </ul>
      </div>
